...
- **Chat / messaging, both tools:** for each active project, check whichever chat tool `memory/engagements.md` records for it, Slack or Google Chat, don't default to one and skip the other. Within each: unanswered mentions of the user; DMs where the other person sent last; VIP messages not yet actioned (VIPs from `memory/day-to-day.md`). Dedup by message timestamp.
- **Email:** new mail needing a response or action. Dedup by message ID.
- **ClickUp activity:** comments, status changes, and mentions on tasks the user is watching or assigned to, across every active project's Execution Tasks and Bugs & Change Requests lists. This is a distinct source from the board itself, it's new information arriving, not the board being cleaned.
