...
**A tag fires regardless of assignment, and end-of-day-check reads ClickUp now too.** The original capture logic scoped ClickUp comments and mentions behind "tasks you're assigned to or watching," which misses the actual signal for someone rarely assigned tasks directly, most team updates arriving as a tagged comment on a task they have no other relationship to. Fixed: a tag or mention fires on its own, independent of assignment or watch status; assignment/watching is now a separate, smaller, additional condition. `end-of-day-check` also gained ClickUp as a source outright, it only read Slack and email before, which meant the handoff-risk pass was blind to exactly the channel most likely to carry a real handoff item.
